const getTailwindClass = () => {
  let raw = [...document.querySelectorAll('.css_code_panel--cssLine--3U0SJ')].map((item) => item.innerText.slice(0, -1).split(': '))
  console.log(raw)
...
const createSnippet = () => {
  const parent = document.querySelector('.css_code_panel--cssCodeContent--3ffb6')
...
const init = () => {
  ...
  // Tab click
  document.querySelector('.pages_panel--tab--3s1Y5[data-label="Inspect"]').addEventListener('click', (event) => {
